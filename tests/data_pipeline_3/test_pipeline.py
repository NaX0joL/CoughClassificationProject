...
def test_pipeline_returns_data_module_for_fold() -> None:
    train_records = [SourceRecord({}, Path("train.wav"))]
    validation_records = [SourceRecord({}, Path("validation.wav"))]
    test_records = [SourceRecord({}, Path("test.wav"))]
    source_reader = Mock()
    ...
    data_module = pipeline.get_data_module(index=0, batch_size=2)
